...
PROJECT_BRIEF = (
    "RansomEye is an AI-powered alert correlation and deduplication engine for SRE teams "
...
def _build_messages(context: dict[str, Any], question: str, conversation: list[ConversationTurn]) -> list[dict[str, str]]:
    system_prompt = (
        "You are an SRE Incident Response Assistant for RansomEye. "
...
def _call_chat_api(api_key: str, url: str, model: str, messages: list[dict[str, str]]) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Mozilla/5.0 (compatible; RansomEye-Backend/1.0)",
        },
        method="POST",
    )
...
def _build_workspace_messages(
    ...
    system_prompt = (
        "You are an SRE Workspace Assistant for RansomEye — an AI-powered alert "
