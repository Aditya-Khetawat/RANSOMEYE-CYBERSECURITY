...
def _initial_load() -> None:
    # A prior run's batch survives a backend restart in ransomeye.db — reload
...
def _initial_load_ransomeye() -> None:
    # So GET /ransomeye/state has data on first load without waiting for the
    # frontend to trigger a demo scenario — same reasoning as _initial_load()
    # above for the RansomEye pipeline.
...
app = FastAPI(title="RansomEye — AI-Powered Real-Time Ransomware Early Warning System", lifespan=lifespan)
