...
1. db.py binds a SQLAlchemy engine/sessionmaker to a fixed file
   (backend/ransomeye.db) at import time. Tests must never touch that real
...
def client(isolated_db, monkeypatch):
    ...
    # the background initial-load thread would otherwise race it. Same deal
    # for RansomEye's own initial load: it populates a module-level _state
    # dict in app.ransomeye.api that isn't reset between tests the way the
    # DB is, so an un-neutered background load here would leak a loaded
