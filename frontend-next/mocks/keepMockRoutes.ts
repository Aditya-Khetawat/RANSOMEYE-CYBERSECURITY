 ...
 * in the RansomEye FastAPI backend. Requests to these are served from demo data
 ...
 *
 * DANGER: never add a prefix that shadows a real RansomEye endpoint. The
...
const ROUTES: [string, Resolver][] = [
  ...
  [
    "preset",
    (path) =>
      // /preset/{name}/alerts returns alerts, not presets. RansomEye's own
  ...
  // RansomEye's /alerts/{id}/ack|assign|dismiss|escalate, which stay on
  // FastAPI. RansomEye's own alert views use /pipeline, not this.
